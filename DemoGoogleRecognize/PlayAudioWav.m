...
{
    AudioInfo* userInfo = (AudioInfo*)inUserData;
    PlayAudioWav* player = (PlayAudioWav *)userInfo.playAudio;
...
-(void) audioQueueOutputWithQueue :(AudioInfo*)userInfo : (AudioQueueRef)audioQueue queueBuffer:(AudioQueueBufferRef)audioQueueBuffer
{
    ...
    [headFileData autorelease];
    [playDataDict autorelease];
...
{
    NSString *path = [[NSBundle mainBundle]pathForResource:strSongName ofType:strSongType];
    OSStatus status = AudioFileOpenURL((CFURLRef)[NSURL fileURLWithPath:path], kAudioFileReadPermission, kAudioFileWAVEType, audioFile);
...
- (AudioInfo*)CreateAudioFile :(NSString *)strSongName :(NSString *)strSongType 
{
    ...
    AudioQueueNewOutput(&RecordFormat, AudioCallbackFromFile, userInfo,
...
- (AudioInfo*)CreateAudioBuffer : (NSData*) nsAudioData : (AudioStreamBasicDescription) RecordFormat
{
    ...
    memcpy(userInfo.RecordFormat, &RecordFormat, sizeof(RecordFormat));
    AudioQueueRef audioQueue;
    AudioQueueNewOutput(&RecordFormat, AudioCallbackFromBuffer, userInfo,nil, nil, 0, &audioQueue);
